...
import { Plus, X, Check, Play, CheckSquare } from "lucide-react";
...
export const Auditing = () => {
  ...
  const handleOpenVerify = (detail, cycle) => {
  ...
  const handleOpenCycleDetail = async (id) => {
    try {
  ...
  const handleStartCycle = async (id) => {
    try {
      ...
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Schedule equipment inspections, assign auditors, and run
            verification audits
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-bold text-navy mb-2">Audit History</h3>
            <div className="space-y-4">
              {cycles.length === 0 ? (
                <p className="text-xs text-gray-500 text-center py-6 bg-white border border-gray-200 shadow-sm rounded-2xl">
                    ...
                    <div
                      ...
                      className={`p-4 rounded-2xl border transition-all cursor-pointer ${
                        isCurrentActive
                          ...
                          : "bg-white border border-gray-200 shadow-sm border-gray-200 hover:border-brand-500/20"
                      }`}
                    >
                      <div className="flex justify-between items-start">
                        <h4 className="font-bold text-navy text-xs">
                          {cycle.name}
                        </h4>
                        <span
                      ...
                      <div className="flex gap-2 mt-4 pt-3 border-t border-white/5">
                        {cycle.status === "DRAFT" && (
                          <button
                            ...
                            className="flex items-center gap-1 px-3 py-1 bg-brand-500/10 hover:bg-brand-500 text-brand-400 hover:text-navy rounded-lg text-[9px] font-bold border border-brand-500/20 transition-all"
                          >
                            <Play size={10} />
                        ...
                        {cycle.status === "IN_PROGRESS" && (
                          <button
                            ...
                            className="flex items-center gap-1 px-3 py-1 bg-emerald-500/10 hover:bg-emerald-500 text-emerald-400 hover:text-navy rounded-lg text-[9px] font-bold border border-emerald-500/20 transition-all"
                          >
                            <Check size={10} />
                            <span>Complete</span>
                          </button>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {!activeCycle ? (
              <div className="bg-white border border-gray-200 shadow-sm p-12 rounded-3xl border border-gray-200 text-center text-gray-500 flex flex-col items-center justify-center">
                <CheckSquare size={36} className="text-gray-600 mb-2" />
                <p className="text-xs">
                  Select an audit cycle from the list to view and record asset
                  inspection statuses.
                </p>
              </div>
            ) : (
              <div className="bg-white border border-gray-200 shadow-sm rounded-3xl border border-gray-200 overflow-hidden animate-fade-in">
                <div className="p-6 border-b border-gray-200 bg-gray-50 flex justify-between items-center flex-wrap gap-2">
                  <div>
                  ...
                  {activeCycle.status === "IN_PROGRESS" && (
                    <button
                      ...
                      className="px-3.5 py-1.5 bg-emerald-500/15 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-500 hover:text-navy rounded-lg text-[10px] font-bold transition-all"
                    >
                      Close Audit Cycle
                    </button>
                  )}
                          ...
                          >
                            ...
                            <td className="py-3.5 px-6 text-right">
                              {activeCycle.status === "IN_PROGRESS" && (
                                <button
                                  onClick={() =>
                                    handleOpenVerify(det, activeCycle)
                                  }
                                  className="px-2.5 py-1.5 bg-brand-500/10 border border-brand-500/20 hover:bg-brand-500 text-brand-400 hover:text-navy rounded-lg text-[9px] font-semibold transition-all"
                                >
                                  Inspect
                                </button>
                              )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white border border-gray-200 p-6 rounded-3xl border border-gray-200 shadow-2xl relative animate-fade-in">
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-4 text-xs">
              ...
              <div className="space-y-1">
                <label className="text-gray-700 font-semibold">Auditor *</label>
                <select
                  value={auditorId}
              ...
              <div className="space-y-1">
                <label className="text-gray-700 font-semibold">
                  Target Scope (Filter Department)
                </label>
                <select
                  value={departmentId}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white border border-gray-200 p-6 rounded-3xl border border-gray-200 shadow-2xl relative animate-fade-in">
            ...
            <form onSubmit={handleVerifySubmit} className="space-y-4 text-xs">
              <div className="p-3 bg-gray-50 border border-white/5 rounded-xl text-gray-500">
                <p>
                  Asset:{" "}
                  <span className="font-bold text-navy">
                    {selectedDetail.asset.name}
                  </span>
                </p>
                <p className="mt-1">
                  Tag:{" "}
                  <span className="font-mono text-navy">
                    {selectedDetail.asset.assetTag}
                  </span>
                </p>
              </div>
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  className="w-full px-4 py-2.5 rounded-xl bg-white border border-gray-300 focus:border-navy focus:ring-1 focus:ring-navy outline-none"
                >
                  <option value="VERIFIED" className="bg-surface-bg">
                    Verified (Operational/Available)
                  </option>
                  <option value="DAMAGED" className="bg-surface-bg">
                    Damaged (Needs repair maintenance)
                  </option>
                  <option value="MISSING" className="bg-surface-bg">
                    Missing (Mark as lost)
                  </option>
                  <option value="DISPOSED" className="bg-surface-bg">
                    Disposed (Retire asset)
                  </option>
                </select>
              </div>
