  ...
  const filteredLogs = logs.filter((log) => {
    return (
      ...
      (log.user.employee?.name &&
        log.user.employee.name.toLowerCase().includes(search.toLowerCase()))
    );
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div>
          <h1 className="text-xl font-bold text-navy tracking-wider  flex items-center gap-2">
            <History className="text-brand-400" size={22} />
            <span>System Activity Logs</span>
          </h1>
          <p className="text-xs text-gray-500 mt-1">
            Audit administrative operations, configuration changes, logins, and
            approvals
          </p>
        </div>
        <button
          onClick={fetchLogs}
          className="p-2.5 rounded-xl border border-gray-200 hover:bg-gray-50 text-gray-500 hover:text-navy transition-colors"
          title="Refresh Logs"
        >
          <RefreshCw size={16} />
        </button>
      ...
      {/* SEARCH */}
      <div className="bg-white border border-gray-200 shadow-sm p-4 rounded-2xl border border-gray-200 max-w-md">
        <div className="relative">
          <input
            ...
            className="w-full pl-4 pr-4 py-2 rounded-xl bg-white border border-gray-300 focus:border-navy focus:ring-1 focus:ring-navy outline-none text-xs"
          />
        </div>
      </div>
                    ...
                    >
                      ...
                      <td className="py-3.5 px-6">
                        <div className="font-bold text-navy">
                          {log.user.employee?.name || "System Account"}
                        </div>
                      ...
                      <td
                        className="py-3.5 px-6 max-w-sm truncate"
                        title={log.details}
                      >
                        {log.details}
                      </td>
                      ...
                      <td
                        className="py-3.5 px-6 font-mono text-[9px] text-gray-500 max-w-xs truncate"
                        title={log.browser}
                      >
                        {log.browser || "Unknown"}
                      </td>
