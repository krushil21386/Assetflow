...
import { Wrench, Plus, X, Laptop, User, Calendar, AlertCircle, Check, Play, ShieldAlert } from 'lucide-react';
...
export const Maintenance: React.FC = () => {
  ...
  const handleOpenUpdate = (ticket: MaintenanceRequest) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold text-white tracking-wider glow-text">Maintenance Tickets</h1>
          <p className="text-xs text-gray-400 mt-1">Report asset faults, monitor repairs, assign technical personnel, and audit costs</p>
        </div>
        <button 
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-dark-border">
                {tickets.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="text-center py-8 text-gray-500">No reported maintenance logs.</td>
                  </tr>
                ) : (
                  tickets.map((t) => (
                    <tr key={t.id} className="text-gray-300 hover:bg-white/5 transition-colors">
                      <td className="py-4 px-6 font-bold text-white">
                        ...
                        <span className="font-mono text-[9px] text-gray-500 font-normal">{t.asset.assetTag}</span>
                      </td>
                      <td className="py-4 px-6 max-w-xs truncate">{t.issue}</td>
                      <td className="py-4 px-6">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-semibold border ${
                          t.priority === 'CRITICAL' ? 'bg-red-500/10 text-red-500 border-red-500/20' :
                      ...
                      <td className="py-4 px-6">{t.assignedTo?.name || 'Unassigned'}</td>
                      <td className="py-4 px-6 font-mono font-bold">₹{t.cost.toFixed(2)}</td>
                      <td className="py-4 px-6">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-semibold border ${
                      ...
                      <td className="py-4 px-6 text-right">
                        {isManagerOrAdmin && (
                          <button
                            ...
                            className="px-2.5 py-1.5 bg-brand-500/10 hover:bg-brand-500 text-brand-400 hover:text-white border border-brand-500/20 rounded-lg text-[10px] font-semibold transition-colors"
                          >
                            Update Progress
                          </button>
                        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md glass-panel p-6 rounded-3xl border border-dark-border shadow-2xl relative animate-fade-in">
            <div className="flex justify-between items-center pb-4 border-b border-dark-border mb-6">
              <h3 className="text-base font-bold text-white">Log Fault Report</h3>
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-4 text-xs">
              <div className="space-y-1">
                <label className="text-gray-300 font-semibold">Select Faulty Asset *</label>
                <select
                  value={assetId}
              ...
              <div className="space-y-1">
                <label className="text-gray-300 font-semibold">Fault Issue Details *</label>
                <textarea
                  value={issue}
                  onChange={(e) => setIssue(e.target.value)}
                  placeholder="Describe screen flickers, ignition faults, body cracks..."
                  className="w-full px-4 py-2 rounded-xl glass-input h-24"
                  required
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  className="w-full px-4 py-2.5 rounded-xl glass-input"
                >
                  <option value="LOW" className="bg-[#080b11]">Low (Minor issue)</option>
                  <option value="MEDIUM" className="bg-[#080b11]">Medium (Slowing productivity)</option>
                  <option value="HIGH" className="bg-[#080b11]">High (Stopped operations)</option>
                  <option value="CRITICAL" className="bg-[#080b11]">Critical (Safety risk/Severe failure)</option>
                </select>
              </div>
      ...
      {updateModalOpen && selectedTicket && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md glass-panel p-6 rounded-3xl border border-dark-border shadow-2xl relative animate-fade-in max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center pb-4 border-b border-dark-border mb-6">
              <h3 className="text-base font-bold text-white">Update Ticket Progress</h3>
              <button onClick={() => setUpdateModalOpen(false)} className="text-gray-400 hover:text-white">
                <X size={18} />
              </button>
            </div>
            ...
            <form onSubmit={handleUpdateSubmit} className="space-y-4 text-xs">
              <div className="p-3 bg-white/5 border border-white/5 rounded-xl space-y-1">
                <p className="text-gray-400">Asset: <span className="font-bold text-white">{selectedTicket.asset.name}</span></p>
                <p className="text-gray-400">Issue: <span className="text-white italic">{selectedTicket.issue}</span></p>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-gray-300 font-semibold">Workflow Status *</label>
                  <select
                    value={status}
                    onChange={(e) => setStatus(e.target.value)}
                    className="w-full px-4 py-2.5 rounded-xl glass-input"
                  >
                    <option value="PENDING" className="bg-[#080b11]">Pending</option>
                    <option value="APPROVED" className="bg-[#080b11]">Approve Request</option>
                    <option value="TECHNICIAN_ASSIGNED" className="bg-[#080b11]">Assign Tech</option>
                    <option value="IN_PROGRESS" className="bg-[#080b11]">In Progress</option>
                    <option value="COMPLETED" className="bg-[#080b11]">Mark Completed</option>
                    <option value="CLOSED" className="bg-[#080b11]">Close Ticket</option>
                    <option value="REJECTED" className="bg-[#080b11]">Reject / Cancel</option>
                  </select>
                </div>

                <div className="space-y-1">
                  <label className="text-gray-300 font-semibold">Assign Technician</label>
                  <select
                    value={assignedToId}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-gray-300 font-semibold">Maintenance Cost (₹)</label>
                  <input
                    type="number"
                    step="0.01"
                    value={cost}
              ...
              <div className="space-y-1">
                <label className="text-gray-300 font-semibold">Transition Status Notes (History log) *</label>
                <textarea
                  value={notes}
