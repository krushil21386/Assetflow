...
export const createDepartment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existing) {
      return res.status(400).json({ message: 'Department already exists with this name' });
    }
    ...
    await logActivity(userId, 'CREATE_DEPARTMENT', `Created department ${name}`, req);
...
export const updateDepartment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await logActivity(userId, 'UPDATE_DEPARTMENT', `Updated department ${updated.name}`, req);
...
export const deleteDepartment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (dept.employees.length > 0) {
      return res.status(400).json({ message: 'Cannot delete department with assigned employees. Reassign them first.' });
    }

    if (dept.assets.length > 0) {
      return res.status(400).json({ message: 'Cannot delete department with registered assets. Reassign them first.' });
    }

    if (dept.subDepartments.length > 0) {
      return res.status(400).json({ message: 'Cannot delete department with sub-departments. Delete or re-parent sub-departments first.' });
    }

    await prisma.department.delete({ where: { id: deptId } });

    await logActivity(userId, 'DELETE_DEPARTMENT', `Deleted department ${dept.name}`, req);
