...
import { PrismaClient, MaintenancePriority, MaintenanceStatus, AssetStatus } from '@prisma/client';
...
export const getMaintenance = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (priority) filters.priority = priority as MaintenancePriority;
...
export const createMaintenanceRequest = async (req: Request, res: Response) => {
  ...
  if (!employeeId) {
    return res.status(400).json({ message: 'User must be linked to an employee profile to request maintenance' });
  }
  ...
  try {
    ...
    const filePaths = files.map(file => `uploads/maintenance/${file.filename}`);
    ...
    await logActivity(userId, 'CREATE_MAINTENANCE', `Raised maintenance request for ${asset.name}`, req);
...
export const updateMaintenanceRequest = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const nextStatus = (status as MaintenanceStatus) || existing.status;
    const nextAssignedId = assignedToId ? parseInt(assignedToId, 10) : existing.assignedToId;
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.maintenanceHistory.create({
        data: {
          requestId: reqId,
          status: nextStatus,
          notes: notes || `Status updated to ${nextStatus.replace('_', ' ').toLowerCase()}`,
          updatedById: employeeId,
        },
      });

      // 3. Asset Status Toggles
      if (nextStatus === 'APPROVED' || nextStatus === 'IN_PROGRESS' || nextStatus === 'TECHNICIAN_ASSIGNED') {
        // Change asset status to MAINTENANCE
        await tx.asset.update({
          where: { id: existing.assetId },
          data: { status: 'MAINTENANCE' },
        });
      } else if (nextStatus === 'COMPLETED' || nextStatus === 'CLOSED' || nextStatus === 'REJECTED') {
    ...
    await logActivity(userId, 'UPDATE_MAINTENANCE', `Updated maintenance ticket ID ${reqId} to ${nextStatus}`, req);
