...
export const getDashboardSummary = async (req, res) => {
  ...
  try {
    ...
    const availableAssetsCount = await prisma.asset.count({
      where: { status: "AVAILABLE" },
    });
    const allocatedAssetsCount = await prisma.asset.count({
      where: { status: "ALLOCATED" },
    });
    const maintenanceCount = await prisma.asset.count({
...
export const getReportsAssets = async (req, res) => {
  try {
    ...
    const categoryValue = {};
    assets.forEach((a) => {
      categoryCounts[a.category.name] =
        (categoryCounts[a.category.name] || 0) + 1;
      categoryValue[a.category.name] =
        (categoryValue[a.category.name] || 0) + a.purchaseCost;
    });
    ...
    const depreciatedAssets = assets.map((a) => {
      ...
      return {
        ...
        depreciatedAmount:
          Math.round((a.purchaseCost - currentVal) * 100) / 100,
      };
    });
    ...
    const totalCurrentValue = depreciatedAssets.reduce(
    ...
    return res.status(200).json({
      ...
      statusBreakdown: Object.keys(statusCounts).map((k) => ({
        name: k,
        count: statusCounts[k],
      })),
      categoryBreakdown: Object.keys(categoryCounts).map((k) => ({
        name: k,
        count: categoryCounts[k],
        value: categoryValue[k],
      })),
      conditionBreakdown: Object.keys(conditionCounts).map((k) => ({
        name: k,
        count: conditionCounts[k],
      })),
      depreciatedAssets,
    });
  } catch (error) {
...
export const getReportsMaintenance = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      priorityBreakdown: Object.keys(priorityCounts).map((k) => ({
        name: k,
...
export const getReportsBookings = async (req, res) => {
  try {
    ...
    bookings.forEach((b) => {
      ...
      assetUtilization[b.asset.name] =
        (assetUtilization[b.asset.name] || 0) + 1;
    });
    ...
    return res.status(200).json({
      ...
      utilizationBreakdown: Object.keys(assetUtilization).map((k) => ({
        name: k,
        count: assetUtilization[k],
      })),
      bookings,
    });
  } catch (error) {
