...
import { PrismaClient, AssetStatus, TransferStatus } from '@prisma/client';
import { logActivity } from '../utils/logger';
import { createNotification, notifyRoles } from '../utils/notifications';

const prisma = new PrismaClient();

export const allocateAsset = async (req: Request, res: Response) => {
  const { assetId, employeeId, expectedReturnDate, conditionBefore, notes } = req.body;
  const operatorUserId = (req as any).user.id;
  const operatorEmployeeId = (req as any).user.employeeId;

  if (!assetId || !employeeId || !conditionBefore) {
    return res.status(400).json({ message: 'Asset, Employee and Condition Before are required' });
  }

  if (!operatorEmployeeId) {
    return res.status(400).json({ message: 'Operator does not have an employee profile' });
  }
  ...
  try {
    ...
    if (asset.status === 'ALLOCATED') {
      return res.status(400).json({ message: 'Cannot allocate: Asset is already allocated' });
    }
    if (asset.status === 'MAINTENANCE') {
      return res.status(400).json({ message: 'Cannot allocate: Asset is currently under maintenance' });
    }
    if (['RETIRED', 'DISPOSED', 'LOST'].includes(asset.status)) {
      return res.status(400).json({ message: `Cannot allocate: Asset is ${asset.status.toLowerCase()}` });
    }
    ...
    await logActivity(operatorUserId, 'ALLOCATE_ASSET', `Allocated asset ${asset.name} to ${employee.name}`, req);
...
export const returnAsset = async (req: Request, res: Response) => {
  ...
  if (!allocationId || !conditionAfter) {
    return res.status(400).json({ message: 'Allocation ID and Condition After are required' });
  }
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const nextStatus: AssetStatus = (conditionAfter === 'DAMAGED' || conditionAfter === 'POOR')
        ? 'MAINTENANCE'
        : 'AVAILABLE';
    ...
    await logActivity(operatorUserId, 'RETURN_ASSET', `Returned asset ${allocation.asset.name} from ${allocation.employee.name}`, req);
...
export const requestTransfer = async (req: Request, res: Response) => {
  ...
  if (!assetId || !toDepartmentId) {
    return res.status(400).json({ message: 'Asset and Target Department are required' });
  }
  ...
  try {
    ...
    if (!asset.departmentId) {
      return res.status(400).json({ message: 'Asset is not assigned to any department. Allocate or update it first.' });
    }

    if (asset.departmentId === targetDeptId) {
      return res.status(400).json({ message: 'Asset is already in the target department' });
    }
    ...
    await logActivity(operatorUserId, 'REQUEST_TRANSFER', `Requested transfer of ${asset.name} to department ID ${targetDeptId}`, req);
...
export const approveTransferHOD = async (req: Request, res: Response) => {
  const { id } = req.params;
  const operatorUserId = (req as any).user.id;
  const operatorEmployeeId = (req as any).user.employeeId;
  ...
  try {
    ...
    if (transfer.status !== 'REQUESTED') {
      return res.status(400).json({ message: `Cannot approve: Transfer is in status ${transfer.status}` });
    }
    ...
    if (fromDept?.headId !== operatorEmployeeId && (req as any).user.role !== 'Admin') {
      return res.status(403).json({ message: 'Only the Department Head of the source department (or Admin) can give HOD approval.' });
    }
    ...
    await notifyRoles(['Admin', 'Asset Manager'], 'Transfer Request Pending manager approval', `Transfer of "${transfer.asset.name}" approved by HOD, pending final inventory approval.`);

    await logActivity(operatorUserId, 'APPROVE_TRANSFER_HOD', `Approved HOD stage of transfer for asset ${transfer.asset.name}`, req);
...
export const approveTransferManager = async (req: Request, res: Response) => {
  const { id } = req.params;
  const operatorUserId = (req as any).user.id;
  ...
  try {
    ...
    if (transfer.status !== 'APPROVED_BY_HOD') {
      return res.status(400).json({ message: 'HOD approval is required before Asset Manager approval' });
    }
    ...
    await logActivity(operatorUserId, 'APPROVE_TRANSFER_MANAGER', `Completed transfer of asset ${transfer.asset.name} to department ID ${transfer.toDepartmentId}`, req);
...
export const rejectTransfer = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const updated = await prisma.assetTransfer.update({
      ...
      data: {
        status: 'REJECTED',
        remarks: reason ? `${transfer.remarks || ''} [Rejected: ${reason}]` : transfer.remarks,
      },
    });
    ...
    await logActivity(operatorUserId, 'REJECT_TRANSFER', `Rejected transfer of asset ${transfer.asset.name}`, req);
    return res.status(200).json(updated);
  } catch (error) {
