...
async function main() {
  ...
  // 3. Seed Categories
  const categories = [
    { name: 'Laptops', description: 'Company laptops and notebooks', warrantyPeriod: 36, depreciationYears: 3, bookable: false },
    { name: 'Monitors', description: 'External display screens', warrantyPeriod: 24, depreciationYears: 5, bookable: false },
    { name: 'Chairs', description: 'Ergonomic office seating', warrantyPeriod: 12, depreciationYears: 7, bookable: false },
    { name: 'Vehicles', description: 'Company vehicles', warrantyPeriod: 60, depreciationYears: 8, bookable: true },
    { name: 'Printers', description: 'Network printing machines', warrantyPeriod: 12, depreciationYears: 5, bookable: false },
    { name: 'Projectors', description: 'Meeting room projectors', warrantyPeriod: 24, depreciationYears: 4, bookable: true },
    { name: 'Meeting Rooms', description: 'Shared physical meeting spaces', warrantyPeriod: 0, depreciationYears: 0, bookable: true },
  ];
  ...
  const usersToSeed = [
    { email: 'admin@assetpilot.local', name: 'Administrator', role: 'Admin', code: 'EMP001', dept: 'Executive', des: 'IT Administrator' },
    { email: 'manager@assetpilot.local', name: 'Asset Manager', role: 'Asset Manager', code: 'EMP002', dept: 'Operations', des: 'Inventory Supervisor' },
    { email: 'depthead@assetpilot.local', name: 'Department Head', role: 'Department Head', code: 'EMP003', dept: 'Engineering', des: 'Engineering Director' },
    { email: 'employee@assetpilot.local', name: 'Test Employee', role: 'Employee', code: 'EMP004', dept: 'Engineering', des: 'Software Engineer' },
  ];
  ...
  for (const a of assetsToSeed) {
    const existingAsset = await prisma.asset.findUnique({ where: { assetTag: a.assetTag } });
    if (existingAsset) {
